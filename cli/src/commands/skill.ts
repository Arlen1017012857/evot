...
const SKILLS_DIR = getSkillsDirs()[0]!;
...
export function skillList(): string {
  return skillListFromDirs(getSkillsDirs());
}
...
export function parseGitHubSource(input: string): GitHubSource {
  ...
  throw new Error(
    `Invalid source: ${trimmed}. Use owner/repo or a GitHub URL.`,
  );
}
...
export async function skillInstall(
  ...
  try {
    ...
    const ghToken = Bun.spawnSync(["gh", "auth", "token"], {
      stdout: "pipe",
      stderr: "pipe",
    });
    const token =
      ghToken.exitCode === 0 ? ghToken.stdout.toString().trim() : "";
    ...
    const download = Bun.spawnSync(
      [
        ...
        "-o",
        tarFile,
        `https://api.github.com/repos/${parsed.repo}/tarball/${gitRef}`,
      ],
    ...
    if (extract.exitCode !== 0) {
      throw new Error(
        `failed to extract tarball: ${extract.stderr.toString()}`,
      );
    }
    ...
    if (forkedAgent && installed.length === 1) {
      ...
      progress?.(
        `  installed ${installed.length} skills; use the skill tool to explore each one`,
        "info",
      );
    }
